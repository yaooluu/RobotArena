...
public class PathFinding {
	...
	private double Heuristic(Graph graph, int node, int goal) {
		...
		//roughly based on line distance
		int[] starts = graph.getRowColFromNode(node);
		int[] ends = graph.getRowColFromNode(goal);
		
		double h = Math.sqrt(Math.pow(starts[0]-starts[1], 2) + Math.pow(ends[0]-ends[1], 2));
		return h;	
	}
	...
	public static List<Integer> Dijkstra(Graph graph, int start, int end) {//System.out.println("Calculating: "+start+", "+end);
		...
		for(int i=0;i<csf.length;i++) csf[i] = Double.MAX_VALUE - 1;
		csf[start] = 0;
		...
		while(open.size() > 0) {
			...
			for(int i=0;i<connections.size();i++) {
				...
				double endNodeCost = csf[current] + connections.get(i).getCost();
	...
	public List<Integer> AStar(Graph graph, int start, int end) {
		...
		while(open.size() > 0) {
			...
			for(int i=0;i<connections.size();i++) {
				...
				if(endNodeCost < csf[endNode]) {
					csf[endNode] = endNodeCost;
					etc[endNode] = Heuristic(graph, current, end) + csf[endNode];
					prev[endNode] = current;
